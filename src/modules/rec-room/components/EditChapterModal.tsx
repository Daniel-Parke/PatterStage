            ...
            {WORD_COUNT_OPTIONS.map((opt) => (
              <button key={opt.id} onClick={() => onWordCountChange(opt.id)}
                className={`px-2 py-1 rounded text-[10px] font-mono border transition-all ${
                  wordCount === opt.id ? "border-neon-purple/40 bg-neon-purple/15 text-neon-purple" : "border-white/8 text-white/30 hover:text-white/50"
                }`}>{opt.label}</button>
            ))}
          </div>
        </div>
            ...
            {[2, 3, 4, 5].map(n => (
              <button key={n} onClick={() => onCountChange(n)}
                className={`px-3 py-1.5 rounded-lg text-xs font-mono border transition-all ${
                  count === n ? "border-neon-purple/40 bg-neon-purple/15 text-neon-purple" : "border-white/8 text-white/30 hover:text-white/50"
                }`}>{n}</button>
            ))}
          </div>
        </div>
