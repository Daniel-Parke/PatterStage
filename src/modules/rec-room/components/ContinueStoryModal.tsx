            ...
            {[2, 3, 4, 5].map(n => (
              <button key={n} onClick={() => onCountChange(n)}
                className={`px-3 py-1.5 rounded-lg text-xs font-mono border transition-all ${
                  count === n ? "border-green-500/40 bg-green-500/15 text-green-400" : "border-white/8 text-white/30 hover:text-white/50"
                }`}>{n}</button>
            ))}
          </div>
        </div>
            ...
            {WORD_COUNT_OPTIONS.map((opt) => (
              <button key={opt.id} onClick={() => onWordCountChange(opt.id)}
                className={`px-2 py-1 rounded text-[10px] font-mono border transition-all ${
                  wordCount === opt.id ? "border-green-500/40 bg-green-500/15 text-green-400" : "border-white/8 text-white/30 hover:text-white/50"
                }`}>{opt.label}</button>
            ))}
          </div>
        </div>
