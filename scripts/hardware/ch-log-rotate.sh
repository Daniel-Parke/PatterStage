...
#   CH_HARDWARE_LOG_DIR     ($CH_DATA_DIR/logs)
#   CH_LOG_RETENTION_DAYS   (30)

set -euo pipefail

CH_DATA_DIR="${CH_DATA_DIR:-${CONTROL_HUB_DATA_DIR:-$HOME/control-hub/data}}"
LOG_DIR="${CH_HARDWARE_LOG_DIR:-$CH_DATA_DIR/logs}"
RETENTION="${CH_LOG_RETENTION_DAYS:-30}"

log() { echo "[$(date -Iseconds 2>/dev/null || date)] [ch-log-rotate] $*"; }
