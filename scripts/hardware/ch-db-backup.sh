...
#   CH_DATA_DIR        ($HOME/control-hub/data)
#   CH_DB_BACKUP_DIR   ($CH_DATA_DIR/backups/db)
#   CH_DB_BACKUP_KEEP  (14)

set -euo pipefail

CH_DATA_DIR="${CH_DATA_DIR:-${CONTROL_HUB_DATA_DIR:-$HOME/control-hub/data}}"
DB="$CH_DATA_DIR/control-hub.db"
BACKUP_DIR="${CH_DB_BACKUP_DIR:-$CH_DATA_DIR/backups/db}"
KEEP="${CH_DB_BACKUP_KEEP:-14}"

log() { echo "[$(date -Iseconds 2>/dev/null || date)] [ch-db-backup] $*"; }
